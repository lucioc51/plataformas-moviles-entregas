...
function sumarLista(listaDeNumeros) {
    let sumaTotal = 0;
    listaDeNumeros.forEach((e) => {sumaTotal += e});
    return sumaTotal;
}
...
function triplicarLista(listaDeNumeros) {
    let listaTriplicada = listaDeNumeros.map((e) => {return e*3});
...
function ordenarDeMayorAMenor(listaDeNumeros) {
    let listaOrdenada = invertirLista(listaDeNumeros.sort());
...
function encontrarNumeroMayor(listaDeNumeros) {
    let numeroMayor = ordenarDeMayorAMenor(listaDeNumeros)[0];
    return numeroMayor;
}
...
function ordenarPalabrasPorLongitud(listaDePalabras) {
    let palabrasOrdenadas = listaDePalabras.sort((s1,s2)=>s1.length-s2.length);
...
function encontrarPalabraMasCorta(listaDePalabras) {
    let encontrarPalabraMasCorta = ordenarPalabrasPorLongitud(listaDePalabras);
    let palabraCorta = encontrarPalabraMasCorta.pop();
    return palabraCorta;
}
...
function filtrarSoloPositivos(listaDeNumeros) {
    let filtrarSoloPositivos = ordenarDeMayorAMenor(listaDeNumeros);
    let listaPositivos = filtrarSoloPositivos.filter((e)=>{return e>0});
...
function contarAprobados(listaDeNotas) {
    let contarAprobados = listaDeNotas.filter((e)=>{return e>6});
    return contarAprobados.length;
}
...
function filtrarSoloTruthy(listaDeValores) {
    let listaVerdad = listaDeValores.filter((e)=>{return !!e});
    return listaVerdad;
}
